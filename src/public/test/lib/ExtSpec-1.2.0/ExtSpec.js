...
(function (ExtSpec) {
    ...
    */
    function apply(destination, source) {
        ...
        for(key in source) {
            if(source.hasOwnProperty(key) && key !== 'statics') {
    ...
    */
    function ensureNamespace(namespace, root) {
        ...
        var object = root || window, keys = namespace.split('.'), key, i, len;
    ...
    */
    function warn(warning) {
        ...
        if(ConsoleWrapper.warn) {
            ConsoleWrapper.warn(warning);
            return;
        }
        ConsoleWrapper.log(warning);
    }
    ExtSpec.warn = warn;
            /**
    ...
    (function (ClassManager) {
        ...
        */
        function construct(className) {
            ...
            if(ExtSpec.isObjectType(configuration.statics)) {
                ExtSpec.apply(ctor, configuration.statics);
            }
            ExtSpec.apply(ctor.prototype, configuration);
            return ctor;
        }
        ...
        */
        function create(className) {
            var Constructor = ClassManager.construct(className);
...
(function (Ext) {
    ...
    * @param {Function} [callback] The callback function.
    */
    function define(className, configuration, callback) {
    ...
    (function (Jasmine) {
        ...
        */
        function createSpy(instance, name, suffix, prefix) {
            suffix = suffix || '';
            prefix = prefix || 'get';
            var separator = name.indexOf('@'), accessorName, accessor;
            if(separator > 0) {
                accessorName = name.slice(0, separator);
            } else {
                accessorName = name;
            }
