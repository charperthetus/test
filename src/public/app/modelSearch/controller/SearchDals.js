/* global Ext: false */
Ext.define('Savanna.modelSearch.controller.SearchDals', {
    ...
    createCustomSearchGroupPanel: function(store) {
        return Ext.create('Savanna.modelSearch.view.searchComponent.searchBody.searchDals.CustomSearchGroupForm', {
            store: store
        });
    },
    ...
    resetAllSearchOptions: function(button) {
        var parentView = button.up('model_search_searchdals');
    ...
    dalCheckBoxClicked: function(checkBox) {
        ...
        var parentView = checkBox.up('model_search_searchdals');
        var i = 0;
        var text = 'Select All';
    ...
    resetSingleDal: function (button) {
        var parentView = button.up('model_search_searchDals_searchoptions');
        var dalSearchOptionPanel = parentView.down('model_search_searchDals_custom-search-group-form');
    ...
    init: function () {
        this.control({
            'model_search_searchdals > #searchDalDockedItems #resetAllSearchOptions': {
                click: this.resetAllSearchOptions
            },
            'model_search_searchdals > #searchDalDockedItems #selectAllDals': {
                click: this.selectOrUnselectAllButtonClicked
            },
            'model_search_searchDals_searchoptions > #searchOptionsToggle': {
                click: this.renderCustomOptions
            },
            'model_search_searchDals_searchoptions > #includeDalCheckBox': {
                click: this.dalCheckBoxClicked
            },
            'model_search_searchDals_searchoptions > #resetSingleDal': {
    ...
    renderCustomOptions: function(button) {
        var parentView = button.up('model_search_searchDals_searchoptions');
        var childSearchDalsPanel = parentView.down('model_search_searchDals_custom-search-group-form');

        if (!childSearchDalsPanel) {
            var parentViewId = parentView.itemId;
            var store = this.getStore('Savanna.modelSearch.store.DalSources');
